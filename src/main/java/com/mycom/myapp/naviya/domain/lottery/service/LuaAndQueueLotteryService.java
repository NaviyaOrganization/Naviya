...
public class LuaAndQueueLotteryService implements LotteryService {
    ...
    public String submitLotteryEntry(LotteryEntryRequest request) {
        ...
        // JSON 데이터에 selectedProgram 필드 추가
        String entryData = String.format(
                "{\"name\":\"%s\", \"phone\":\"%s\", \"selectedProgram\":\"%s\"}",
                request.getName(),
                request.getPhone(),
                request.getSelectedProgram()
        );
        ...
        try {
            ...
            // Redis Streams에 프로그램 선택 정보도 포함하여 기록
            writeToLotteryStream(request.getName(), request.getPhone(), request.getSelectedProgram());
    ...
    // Redis Streams 쓰기 메서드 수정
    private void writeToLotteryStream(String name, String phone, String selectedProgram) {
    ...
    public void processLotteryResults() {
        ...
        for (Object entryData : winnersList) {
            try {
                ...
                String selectedProgram = data.get("selectedProgram");

                // selectedProgram 정보를 포함하여 LotteryEntry 생성 및 저장
    ...
    public void updateMaskedEntriesCache() {
        ...
        log.info("조회된 응모 내역 수: {}", entries.size());
