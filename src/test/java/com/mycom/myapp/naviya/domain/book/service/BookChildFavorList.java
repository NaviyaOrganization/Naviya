...
public class BookChildFavorList {
    ...
    @Test
    public void testListBookChildFavor_Success() {
        long childId = 1L; // 자녀 ID 설정
        List<BookDto> mockBooks = new ArrayList<>();
        mockBooks.add(new BookDto(/* 필드 초기화 */)); // 적절한 필드 초기화

        // Mocking the repository method
        when(bookRepository.findBooksLikeByChildId(childId)).thenReturn(mockBooks);

        // Act
        BookResultDto result = bookService.listBookChildFavor(childId);
    ...
    @Test
    public void testListBookChildFavor_EmptyList() {
        long childId = 1L; // 자녀 ID 설정
        // Mocking the repository method to return an empty list
        when(bookRepository.findBooksLikeByChildId(childId)).thenReturn(new ArrayList<>());
    ...
    @Test
    public void testListBookChildFavor_Exception() {
        long childId = 1L; // 자녀 ID 설정
        // 여기에 Mockito에서 예외를 던지도록 설정할 수 있습니다.
        when(bookRepository.findBooksLikeByChildId(childId)).thenThrow(new RuntimeException("Database error"));
