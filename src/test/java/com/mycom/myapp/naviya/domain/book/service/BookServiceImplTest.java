...
public class BookServiceImplTest {
    ...
    @Test
    public void testListBookChildRecntRead_Success() {
        long childId = 1L;
        List<BookDto> mockBooks = new ArrayList<>();
        mockBooks.add(new BookDto(/* 필드 초기화 */));

        // Mocking the repository method
        when(bookRepository.findBooksByChildRecentRead(childId)).thenReturn(mockBooks);

        // Act
        BookResultDto result = bookService.listBookChildRecntRead(childId);
    ...
    @Test
    public void testListBookChildRecntRead_EmptyList() {
        long childId = 1L;

        // Mocking the repository method to return an empty list
        when(bookRepository.findBooksByChildRecentRead(childId)).thenReturn(new ArrayList<>());
    ...
    @Test
    public void testListBookChildRecntRead_Exception() {
        long childId = 1L;

        // 여기에 Mockito에서 예외를 던지도록 설정할 수 있습니다.
        when(bookRepository.findBooksByChildRecentRead(childId)).thenThrow(new RuntimeException("Database error"));
